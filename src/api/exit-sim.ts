...
export interface ExitSimReport {
  ...
  strategies: {
    ...
    scale_out:        { status: 'NOT_IMPLEMENTED' };
    vol_adaptive:     { status: 'NOT_IMPLEMENTED' };
    time_decayed_tp:  { status: 'NOT_IMPLEMENTED' };
...
function computeStaticBaseline(rows: ExitSimRow[], universe: UniversePredicate): GridCell {
  ...
  for (const r of filtered) {
    ...
    if (exitPct === null) {
      ...
      breakdown.momentum_reversal++;  // bucketing TP exits here for simplicity
    }
