...
## 2026-07-12 — Daily review: copy-hotlead-strict's `degrading` flag resolves — recent-window net/trade flips positive, making it the roster's only `promotable_stable` candidate — while `copy-hotlead-strict-hi` keeps easing but stays degrading, the 07-11 book day finalizes to +8.03 SOL (from a −3.22 partial, an ~11.3 SOL favorable swing) and the already-"finalized" 07-09 number moves again (−19.6→−17.55), and the `copy-hotlead-hold30m-live-micro` alarm from yesterday appears resolved (trade count frozen at 324, zero open positions, `diagnose.json` shows zero live strategies)

<!-- SNAPSHOT (machine-readable; do not hand-edit) -->
```json
{
  "date": "2026-07-12",
  "overall": {"n": 7268, "net": -31.071, "drop3": -45.2172, "stress": -105.3104, "open": 21},
  "retired_summary": {"n": 24967, "net": -215.7926},
  "regime_score": 8, "regime_24h": 9, "macro_score": 5, "btc_7d_pct": 1.76,
  "book_daily_today": -0.45,
  "leads": {"n_leads": 173, "hot": 49, "cold": 84},
  "n_promotable_realistic": 2, "n_promotable_stable": 1,
  "strategies": [
    {"id": "copy-hotlead-strict",         "realistic": true,  "n": 828,  "net": 13.150,  "drop3":  6.567,  "stress":  4.358,  "promo_score": 100,  "verdict": "PROMOTE"},
    {"id": "copy-hotlead-strict-hi",      "realistic": true,  "n": 147,  "net":  6.392,  "drop3":  4.192,  "stress":  4.75,   "promo_score": 100,  "verdict": "PROMOTE"},
    {"id": "copy-fable-dip",              "realistic": true,  "n":   5,  "net":  2.320,  "drop3":  0.353,  "stress":  2.222,  "promo_score":  60.4,"verdict": "WATCH"},
    {"id": "copy-fable-leadpullback",     "realistic": true,  "n":   9,  "net":  1.990,  "drop3":  0.019,  "stress":  1.857,  "promo_score":  55.6,"verdict": "WATCH"},
    {"id": "copy-fable-deep",             "realistic": true,  "n":  12,  "net":  1.556,  "drop3": -0.447,  "stress":  1.401,  "promo_score":  51.4,"verdict": "WATCH"},
    {"id": "copy-src-gradspec",           "realistic": true,  "n":   4,  "net":  1.009,  "drop3": -0.167,  "stress":  0.948,  "promo_score":  45.3,"verdict": "WATCH"},
    {"id": "copy-hotlead-early",          "realistic": true,  "n":  47,  "net": -1.645,  "drop3": -3.572,  "stress": -2.100,  "promo_score":  29.4,"verdict": "WATCH"},
    {"id": "copy-fable-freshdip",         "realistic": true,  "n":  42,  "net": -1.070,  "drop3": -3.024,  "stress": -1.481,  "promo_score":  28.4,"verdict": "WATCH"},
    {"id": "copy-src-external",           "realistic": true,  "n":  13,  "net": -0.160,  "drop3": -0.851,  "stress": -0.290,  "promo_score":  22.6,"verdict": "WATCH"},
    {"id": "copy-hotlead-nodump",         "realistic": true,  "n":   5,  "net": -0.460,  "drop3": -0.990,  "stress": -0.500,  "promo_score":  21.0,"verdict": "WATCH"},
    {"id": "copy-hotlead-breadth",        "realistic": true,  "n":   5,  "net": -0.460,  "drop3": -0.990,  "stress": -0.500,  "promo_score":  21.0,"verdict": "WATCH"},
    {"id": "copy-fable-freshdip-bounded", "realistic": true,  "n":   3,  "net": -0.370,  "drop3":  0,      "stress": -0.390,  "promo_score":  20.6,"verdict": "WATCH"},
    {"id": "copy-tp100-sl30-lag",         "realistic": true,  "n":1045,  "net":-32.331,  "drop3":-37.179,  "stress":-42.450,  "promo_score":  40,  "verdict": "KILL"},
    {"id": "copy-conviction-consensus2",  "realistic": false, "n":1545,  "net": 14.175,  "drop3":  1.389,  "stress": -2.022,  "promo_score":  52.4,"verdict": "KEEP"},
    {"id": "copy-tp100-sl30",            "realistic": false, "n":3558,  "net":-35.172,  "drop3":-41.729,  "stress":-71.120,  "promo_score":  20,  "verdict": "KEEP"}
  ]
}
```

**Headline:** The most consequential change today is in the `trend`/`promotable_stable` fields, not the cumulative gates. `copy-hotlead-strict`'s `degrading` flag — first raised 07-10, still present 07-11 — has **resolved**: `recent_net_per_trade` flipped positive (**+0.01002**, up from −0.00423 yesterday, against `prior_net_per_trade` +0.01718), `trend` now reads `"stable"`, and `promotable_stable: true`. It is now the **only** strategy in the roster carrying that flag (`promotion.n_promotable_stable: 1` of `n_promotable: 2`) — the first time since the trend field appeared that any candidate has cleared both the cumulative gates and the recency check simultaneously. `copy-hotlead-strict-hi` continues to ease (recent net/trade improved to **−0.01722**, from −0.04509 yesterday) but is still net-negative on its recent window and remains flagged `degrading`. Separately, the book's partial-to-final reversal pattern produced its most striking read yet: 07-11 finalized at **+8.03 SOL**, a ~11.3 SOL favorable swing from yesterday's −3.22 partial — but 07-09, which this journal already reported as "finalized" at −19.6 SOL, **moved again** to **−17.55 SOL**, meaning even "finalized" daily numbers in `regime.swing.daily` are not fully settled and can still revise days later. Last, yesterday's operational flag on `copy-hotlead-hold30m-live-micro` (real live capital active on a strategy this journal reported killed on 07-05) looks resolved on the data: `n_live` is frozen at exactly 324 (unchanged from yesterday), `n_open: 0` in `live-training.json`'s per-strategy metrics, and `diagnose.json`'s `level5_live_ready.live_strategy_count` reads **0** — consistent with the position having fully wound down via the `strategy_removed` exit path described in the code comment, with no new capital being risked.

**Day-over-day (vs 2026-07-11 SNAPSHOT):**
- **`copy-hotlead-strict` — degrading flag resolved, now the roster's sole `promotable_stable` candidate.** n 820→**828** (+8), net 11.7968→13.150 (+1.353), drop3 5.2144→6.567 (+1.353), stress 3.1147→4.358 (+1.243). Score still 100, monthly run-rate now 28.18 SOL/mo. `recent_net_per_trade` crossed from −0.00423 (yesterday) to **+0.01002** — the first positive recent-window read since the trend field appeared 07-10. `trend: "stable"`, `promotable_stable: true`.
- **`copy-hotlead-strict-hi` — still degrading, still easing.** n 139→**147** (+8), net 5.0501→6.392 (+1.342), drop3 2.854→4.192 (+1.338), stress 3.5175→4.75 (+1.233). Score 100, monthly run-rate 17.43 SOL/mo. `recent_net_per_trade` improved to **−0.01722** (from −0.04509) — third straight easing read, but still negative, so `degrading: true` / `promotable_stable: false` persist. It is now the sole strategy in `promotion.degrading`.
- **`copy-fable-dip`, `copy-fable-leadpullback`, `copy-fable-deep` — all growing, all promo-score movers >10pts.** `copy-fable-dip` n 2→5 (net +1.002, drop3 flipped to **+0.353** from 0, score 48.1→**60.4**), `copy-fable-leadpullback` n 1→9 (net +1.353, score 41.3→**55.6**), `copy-fable-deep` n 5→12 (net +1.180, drop3 improved −0.728→**−0.447** though still negative, score 39.2→**51.4**). All still n≤12 — too early for a verdict, but all three are trending the right direction.
- **`copy-src-gradspec` — completely flat for a 2nd straight day; P1 checkpoint now 2 days overdue.** n, net, drop3, stress, and score are all **byte-identical to yesterday** (n=4, net=1.009, drop3=−0.167, score=45.3) — `discovery_scorecard` funnel shows `candidates: 0, scored: 0` again this cycle. The pre-registered P1 checkpoint (`smart_copyable≥10` by day 5, missed 07-11 at 4/10) is now missed by 2 days with zero movement; the fallback shelve trigger (`<3 by day 7`, due tomorrow 07-13) won't technically fire since `smart_copyable` sits at 4≥3, but two consecutive dead cycles argues for an operator look now rather than waiting out the fallback clock.
- **`copy-hotlead-early` — first improving read after 3 straight worsening ones.** n 46→47 (+1), net −2.33→**−1.645** (+0.685), drop3 −4.157→**−3.572** (+0.585), stress −2.757→−2.100 (+0.657). Promo score 29.2→29.4. Still n<50, but the deepening streak broke for the first time since it entered the journal.
- **`copy-fable-freshdip`, `copy-src-external`, `copy-fable-freshdip-bounded` — all completely flat, zero new trades.** All three unchanged n/net/drop3/score vs yesterday. `copy-src-external` in particular has now had multiple flat cycles despite its `discovery_scorecard` funnel showing 25 `smart_copyable` wallets already scored — the probe itself is barely firing even though the underlying pool isn't empty.
- **Two new strategies appeared overnight: `copy-hotlead-nodump`, `copy-hotlead-breadth`.** Both day-0/1, n=5, identical net/drop3/stress so far (coincidence of small-n — neither gate has triggered yet on their shared first batch). Checked against the redundancy guardrail: `nodump` adds a smart-money net-selling veto (skip if the crowd is net-selling the mint in a 90s window), `breadth` caps copies per lead at 2/hour to broaden the drop3 base across more distinct leads — distinct gates, not duplicates of anything in the roster.
- **`copy-tp100-sl30-lag` — KILL case reconfirmed, 14+ consecutive failing reads.** n 1025→**1045** (+20), net −33.842→−32.331 (+1.511), drop3 −38.69→−37.179 (+1.511), stress −43.723→−42.45 (+1.273). Still failing every gate on every read since entering the journal — the improvement this cycle tracks the book's better days, not a change in the underlying tp100/sl30 exit shape.
- **`copy-conviction-consensus2` (idealized) — biggest score mover, drop3 deepened positive.** n 1536→1545 (+9), net 12.973→14.175 (+1.202), drop3 0.187→**1.389** (+1.202), stress −3.107→−2.022 (+1.085). Promo score 37.3→**52.4** (+15.1). Not a live candidate regardless (no `entryDelaySec`), but the strongest read it's posted since entering the journal.
- **Regime score rose 6→8** ("neutral"→"strong"), `score_24h` rose 7→**9** — both legs improved together for a second straight day, extending 07-11's recovery. Macro dipped 6→5 (tailwind→neutral) as BTC 7d% firmed slightly (1.35%→1.76%) but 1d% turned negative (−0.66%).
- **Book P&L: 07-11 finalizes to +8.03 SOL (from −3.22 partial) — the largest partial-to-final swing recorded (~+11.3 SOL), and the first sign-flip (negative→positive).** More notably, **07-09's already-"finalized" number moved again**: reported as −19.6 on 07-10, confirmed "unchanged, already finalized" on 07-11, now reads **−17.55** today (+2.05 SOL). This is new information: prior entries treated a day as settled once one full cycle passed without it moving, but this shows the settlement window is longer than 24h for at least some days — worth treating ALL `regime.swing.daily` entries as provisional until confirmed stable across multiple cycles, not just the most recent one. Today's partial (07-12): **−0.45 SOL** so far. `daily_mean_sol` over the visible window is −0.28, `daily_std_sol` 12.26.
- **Lead pool count flat but composition softened.** n_leads flat at 173, but **hot 52→49 (−3), cold 81→84 (+3)** — after a full day of zero movement on 07-11, three leads flipped from hot to cold today. Total pool size alone (as reported flat yesterday) masks this quality shift.
- **Bot health:** `diagnose.json` verdict HEALTHY, all levels 1-4 passing, WS connected, last graduation 135s ago. `level5_live_ready.live_strategy_count: 0` — corroborates the hold30m-live-micro wind-down read above. Same transient `gist-sync` "Ref update failed: 500" error class as yesterday, not trading-path-relevant.
- **Operational flag resolved (see live-training.json):** `copy-hotlead-hold30m-live-micro` — `n_live: 324` (unchanged from yesterday's 324, i.e. zero new live trades this cycle), per-strategy metrics show `n_open: 0`, all 324 positions closed (`exit_reason_counts`: follow_shadow 280, stop_loss 38, live_buy_failed 5, sell_parked_unsellable 1 — sums to 324, no `strategy_removed` exits visible but consistent with a prior wind-down). Combined with `diagnose.json` showing zero live strategies, the picture is consistent with this instance having fully unwound rather than continuing to bleed. Total net stayed at −0.7453 SOL (unchanged). Recommend the operator confirm this explicitly (e.g. wallet balance / `COPY_LIVE_ENABLED` state) so future entries can drop this from the standing flag list with certainty, rather than inferring it from unchanged trade counts.

**Week-over-week (last available entries: 07-09 through 07-12):**
- **The two-day `degrading` stretch on both PROMOTE strategies (07-10, 07-11) has partially resolved:** `copy-hotlead-strict` is back to `stable`/`promotable_stable: true` after 2 days flagged; `copy-hotlead-strict-hi` is 3-for-3 on easing reads (−0.05711→−0.04509→−0.01722) but hasn't crossed back to positive yet. If tomorrow's read for `-hi` turns positive, both PROMOTE cases will have fully round-tripped through the degrading-flag mechanism without failing a gate — worth treating as a validation of the "gates first, trend advisory" framing in `promotion.note`.
- **`copy-tp100-sl30-lag` has now failed every gate on every read since entering the journal (14+ consecutive reads).** The unenacted KILL case remains as strong as it has ever been.
- **`copy-src-gradspec` has now had 2 consecutive completely dead cycles** (zero candidates, zero scored, zero trade growth) since missing its P1 checkpoint on 07-11 — the stall is no longer a single-day miss, it's a pattern.
- **Partial-to-final book reversals are now a persistent, multi-cycle phenomenon (5 instances in ~10 days)**, and today adds a new wrinkle: a number already reported "finalized" (07-09) moved again after a full extra cycle. Every `book_daily_today` figure in this journal should be read as a lower/upper bound, not a settled number, until it has held across at least 2 consecutive reads.
- **Lead pool held flat in total count for 2 straight days but flipped 3 wallets hot→cold today** — the "flat" read from 07-11 undersold what was actually a quiet buildup toward today's composition shift.
- **New this cycle: the live-capital discrepancy flagged 07-11 appears to have resolved itself without an explicit operator action being visible in this data** — worth a direct operator confirmation rather than assuming, since the inference relies on an unchanged trade count rather than an explicit "closed" status field.

**Verdicts (proposals — roster changes require operator approval + code edit to `COPY_STRATEGIES`):**

- **PROMOTE (2; `copy-hotlead-strict` now also `promotable_stable`):**
  - `copy-hotlead-strict`: n=828, net=+13.15, drop3=+6.57, stress=+4.36, monthly=28.18 SOL/mo. Score 100, all gates clear, `trend: stable`, `promotable_stable: true` — first time any candidate has cleared both the cumulative bar and the recency check together. Recommend a live-micro test; this is the strongest-evidenced candidate the roster has produced.
  - `copy-hotlead-strict-hi`: n=147, net=+6.39, drop3=+4.19, stress=+4.75, monthly=17.43 SOL/mo. Score 100, all gates clear, but `trend: degrading` persists (recent net/trade −0.01722, 3rd straight easing read but still negative). Recommend a live-micro test with closer monitoring than `-strict` until it also turns `promotable_stable`.

- **KILL (1 carried over, now 14+ consecutive failing reads):**
  - `copy-tp100-sl30-lag`: n=1045, net=−32.33, drop3=−37.18, stress=−42.45. Failed every gate on every read since entering the journal.

- **WATCH:**
  - `copy-src-gradspec`: n=4 (flat for 2nd straight day), P1 checkpoint 2 days overdue with zero funnel movement. Recommend an operator look now rather than waiting for the day-7 fallback (due tomorrow, won't technically trigger since smart_copyable=4≥3).
  - `copy-src-external`: n=13, flat for multiple straight cycles despite a scored pool of 25 `smart_copyable` wallets — probe itself looks under-firing relative to its own funnel.
  - `copy-fable-dip`, `copy-fable-leadpullback`, `copy-fable-deep`: n≤12, all growing and all trending positive/improving — the strongest of the young WATCH strategies, but still far too small for a verdict.
  - `copy-hotlead-early`: n=47, first improving read after 3 worsening ones — too early to call a turn.
  - `copy-fable-freshdip`, `copy-fable-freshdip-bounded`: flat this cycle, still net/drop3 negative overall.
  - `copy-hotlead-nodump`, `copy-hotlead-breadth`: brand new (n=5), distinct gates (smart-distribution veto vs per-lead breadth cap) — not redundant, too early to read.

- **Idealized references (not live candidates):**
  - `copy-conviction-consensus2`: n=1545, net=+14.18, drop3 deepened positive to +1.39 — its strongest read yet, still capped at score ≤80 by having no `entryDelaySec`.
  - `copy-tp100-sl30`: n=3558, net=−35.17. Unchanged verdict, still the deepest negative reference.

- **Operational flag (updated, likely resolved):** `copy-hotlead-hold30m-live-micro` — trade count frozen at 324 (no growth since yesterday), 0 open positions, `diagnose.json` shows 0 live strategies. Consistent with a full wind-down; recommend an explicit operator confirmation to retire this flag with certainty.

**New strategies to try:** None proposed this cycle. Six young strategies are already cooking (`copy-fable-dip/leadpullback/deep/freshdip-bounded`, `copy-hotlead-nodump/breadth`) plus `copy-src-gradspec` awaiting an operator decision on its stalled checkpoint — per the redundancy guardrail, the right move is reading what's already in flight before adding more.

**Operator next step:** Two things worth a look. (1) **`copy-src-gradspec`'s P1 checkpoint is now 2 days overdue with 2 consecutive dead cycles** (zero candidates, zero scored) — worth deciding whether to extend, re-seed, or shelve rather than waiting out the day-7 fallback, which won't technically trigger. (2) **Confirm the `copy-hotlead-hold30m-live-micro` wind-down explicitly** (wallet balance / `COPY_LIVE_ENABLED` state) — the data is consistent with it having fully closed out, but that's inferred from an unchanged trade count, not a status field, and yesterday's entry's "no live capital" framing was already wrong once. Unchanged: enact the long-standing `copy-tp100-sl30-lag` KILL (14+ consecutive failing reads), and consider funding `copy-hotlead-strict`'s live-micro test now that it's cleared both the cumulative gates and the recency check simultaneously for the first time.
