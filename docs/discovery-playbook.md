...
| 2026-07-04 | live_tape (Idea 1) | **PRUNED / FAILS** | probe `copy-src-live-tape` stalled at n=24 for 4+ monitor loops (its wallets rarely trade our copyable graduation universe → can't reach n≥100) and was clearly below the OG control on the trades it did make (net/trade −0.047 vs −0.028; drop3/trade −0.055 vs −0.036). Registry row removed (operator-approved U6); harvester was already OPT-IN/off, so nothing to stop. Revive only with a fresh `probeId` |
...
| — | gradspec (phase-1 handoff 2026-07-05) | COLLECTING | added 2026-07-06: reseed the winner-sniper **forward pre-filter** from the *post-grad-AMM specialist* archetype (`grad_buys` high, `pre_pct ≤ 0.10`, active ≤14d — the `smart-money → timing` panel), attacking the reachability wall that keeps non-OG sources at n≈0 (the 0-30s winner-credit seed reaches ~3 wallets). Harvester `gradspec-harvester.ts` (pure SQL, worker tick, zero RPC) enrolls bar-clearers into `winner_prefilter` with **`origin='gradspec'`** (new column; sub-cap `GRADSPEC_MAX_WATCHING`=75 of the shared 200 slots); the signal set is origin-scoped (`getPrefilterGatedWallets(db, origin)`) so gradspec and winner_sniper stay disjoint. Pre-registered: P1 smart_copyable ≥10 in 5d (shelve if <3 by d7); P2 `BEATS_OG` at n≥100 with drop3/trade > 0 absolute; P3 n≥100 in ~3 weeks. Bar env: `GRADSPEC_MIN_GRAD_BUYS`/`GRADSPEC_MAX_PRE_PCT`/`GRADSPEC_ACTIVE_DAYS` |
| 2026-07-10 | winner_sniper (operator thesis, 3-stage funnel) | **PRUNED / FAILS** | Once the 07-08 watchlist-reserve fix let its wallets subscribe, the probe `copy-src-winner-sniper-v2` collected fast and resolved at full sample: n=148, net −7.17, **−0.048/trade vs the OG control's −0.032** — worse than unselected OG wallets on both metrics. Two rounds of profit-proofing (winner-window hit → forward pre-filter) still don't produce copyable leads: own-trading edge doesn't transfer to a 5s-lagged entry-only mirror (the 06-29 audit's r≈0 finding, reconfirmed the expensive way). Registry row removed 2026-07-10 (harvester + pre-filter were already default-OFF from the 07-09 credit retune); both probeIds (`copy-src-winner-sniper`, `-v2`) are burned. NOTE: `gradspec` shares the (default-off) pre-filter → its funnel is frozen until `PREFILTER_DISABLED=false` |
