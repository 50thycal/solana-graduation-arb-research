...
| `entry-time-matrix.json` | `/api/entry-time-matrix` | All single FILTER_CATALOG filters + top combos × 6 entry checkpoints (T+30, T+60, T+90, T+120, T+180, T+240). Each cell carries `n / opt_tp / opt_sl / opt_avg_ret / opt_win_rate` from `simulateCombo(db, where, entrySec)` — same TP/SL grid + cost model as `/api/best-combos` but anchored on `pct_t<entrySec>`. Per-row `best_entry_sec` + `delta_vs_t30_pp` show whether waiting past T+30 lifts the combo's optimum. Summary block reports the overall distribution + per-filter-group winner. **Caveat:** later entry times have smaller n (only 15.6% of complete observations have full 5s grid as of 2026-05-02); cells with `n < min_n_per_cell` are flagged `low_n: true` and skipped from the `best_entry_sec` pick. Used by the late-entry research handoff (NEXT_SESSION_ENTRY_TIME.md) to decide whether to ship a `entryTimingSec` trading mode. |
